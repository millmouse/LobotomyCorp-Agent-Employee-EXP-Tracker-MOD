    ...
    public class AgentManagerPatch
    {
        ...
        private static string GetBestToWorstStatsListLog(IList<AgentModel> agents)
        {
            ...
            foreach (var agent in agents)
            {
                ...
                // Retrieve stats
                int cubeSpeed = agent.workSpeed;
                int workProb = agent.workProb;
                int attackSpeed = (int)agent.attackSpeed;
                int movementSpeed = (int)agent.movement;
                int maxHP = agent.maxHp;
                int maxMental = agent.maxMental;
